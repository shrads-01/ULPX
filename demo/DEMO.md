...
## 5. Analyst UI
Navigate to `http://localhost:3000/` in a web browser. The Analyst UI allows you to visually explore ingested events, view raw preserved bytes, inspect field provenance, and review the structural inference confidence levels.
...
Run the deterministic benchmark to prove framing, parsing, and exact completeness validations:
```powershell
...
ULPX implements a streaming boundary using `rskafka` in `ulpx-ingest`. Note that this is currently an ingestion adapter only. It does **not** provide consumer-group offset commits, it starts consumption at `StartOffset::Latest`, and it provides **no at-least-once delivery guarantees**. It is documented here as an explicit architectural boundary and limitation, rather than a production-grade distributed consumer demonstration.
