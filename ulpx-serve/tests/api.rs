...
async fn test_get_evidence_success() {
    ...
    store.store(RawEvent::new(id.clone(), b"Hello API \x00\xFF".to_vec(), Source("api-test".into()))).unwrap();

    let app = create_router(Arc::new(store));

    let response = app
        .oneshot(Request::builder().uri("/api/v1/evidence/test-evt").body(Body::empty()).unwrap())
        .await
        .unwrap();
    ...
    let decoded = base64::engine::general_purpose::STANDARD.decode(base64_payload).unwrap();
...
async fn test_get_evidence_not_found() {
    ...
    let response = app
        .oneshot(Request::builder().uri("/api/v1/evidence/missing-evt").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn test_get_interpretation_success() {
    ...
    store.store(RawEvent::new(id.clone(), b"{\"message\":\"hello\"}\n".to_vec(), Source("api-test".into()))).unwrap();

    let app = create_router(Arc::new(store));

    let response = app
        .oneshot(Request::builder().uri("/api/v1/interpretation/test-json").body(Body::empty()).unwrap())
        .await
        .unwrap();
...
async fn test_get_interpretation_integrity_failure() {
    ...
    store.store(RawEvent::new(id.clone(), b"test".to_vec(), Source("api-test".into()))).unwrap();
    ...
    let response = app
        .oneshot(Request::builder().uri("/api/v1/interpretation/test-corrupt").body(Body::empty()).unwrap())
        .await
        .unwrap();
